...
**Flags CLI (solo zero-token `scan.mjs`):**
- `--days N`: Filtrar ofertas por antigüedad (default: 7 días). Ejemplo: `node scan.mjs --days 14`
- `--company NAME`: Escanear una sola empresa. Ejemplo: `node scan.mjs --company Anthropic`
...
8. **Para cada oferta nueva verificada que pase filtros**:
   a. Añadir a `pipeline.md` sección "Pendientes": `- [ ] {posted_date} | {url} | {company} | {title}`
   b. Registrar en `scan-history.tsv`: `{url}\t{date}\t{query_name}\t{title}\t{company}\tadded`
   
   **Nota**: `{posted_date}` (YYYY-MM-DD) viene de la API de cada portal (Greenhouse `created_at`, Ashby `publishedDate`, Lever `createdAt`). Esto permite filtrar por antiguedad y evitar ofertas expiradas.
